...
def generate_qr(qr: QR):
    ...
    if EnvValidator.use_bucket():
        ...
        return {"res": res, "s3": EnvValidator.use_bucket(), "bucket_path": EnvValidator.bucket_endpoint()  }
    else:
        return {"res": resulting_qr_path.replace(EnvValidator.qr_tmp_folder(), ""), "s3": EnvValidator.use_bucket(), "bucket_endpoint": EnvValidator.bucket_endpoint() }
